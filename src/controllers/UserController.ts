...
export class UserController implements Controller{
    ...
    constructor(router){
        this.router = router;
        this.router.get('/update-users-courses', this.updateUsersCourses.bind(this));
        this.userDao = new UserDAO(new ConnectionFactory().getInstance());
        this.userCoursesDao = new UserCourseDAO(new ConnectionFactory().getInstance());
    ...
    async updateUsersCourses(request, response){
      try{
            ...
            const usersCoursesFromProfiles = await this.aluraService.getCoursesFromProfiles(users);
            await this.userCoursesDao.deleteCoursesFromUsers(usersCoursesFromProfiles);
